...
from ..global_vars import BASE_INGREDIENTS
...
def fill_new_ingredients(data, old_size, new_ingredients, fill_on_right=True):
    n_new = len(new_ingredients)
    fill_df = pd.DataFrame(
        np.ones((len(data), n_new), dtype=int), columns=new_ingredients
    )

    if fill_on_right:
        left = data.iloc[:, :old_size]
        right = data.iloc[:, old_size:]
        center = fill_df
        dfs = (left, center, right)
        col_names = list(left.columns) + new_ingredients + list(right.columns)
    else:
        dfs = (fill_df, data)
        col_names = new_ingredients + list(data.columns)
...
if __name__ == "__main__":
    ...
    data_filled = fill_new_ingredients(data, 20, new_ingredients, fill_on_right=True)
