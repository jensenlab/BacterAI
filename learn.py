...
class Agent:
    ...
    def learn(self, batch_size=300, save_models=False):
        ...
        line11 = ax8.stackplot(x_values, grow_proportion, labels=["Grow", "No Grow"])
        ...
        for cycle in range(n_cycles):
            print(f"\nCYCLE {cycle}")
            print(
                f"Data history:  {self.data_history.shape[0]} {self.data_labels_history.shape[0]}"
            )
            ## TRAIN g_(x)
            # x -> g_ -> P(g(x) = 1)
            ...
            if cycle > 0 and batch_train_data.shape[0] > 0:
                _, accuracy, _ = self.get_metrics(
                    batch_train_data.to_numpy(), batch_train_data_labels.to_numpy()
                )
            ...
            (
                precision_answer_space,
                accuracy_answer_space,
                recall_answer_space,
            ) = self.get_metrics(
                minimum_rule_data_x.to_numpy(), minimum_rule_data_y.to_numpy()
            )
            ...
            index_subset = np.random.choice(
                test_data_x.index, size=int(0.1 * test_data_x.shape[0]), replace=False
            )
            ...
            ax6.cla()
            ax6 = fig.add_subplot(427)
            _, _, _, im2 = ax6.hist2d(
                minimum_rule_set_predictions.flatten(),
