...
def make_CDM():
    ...
    with open(file_path, newline="", encoding="utf-8-sig") as csvfile:
        ...
        for row in reader:
            ...
            if short_id not in CDM_groups["amino_acids"]:
                continue
    ...
    with open(file_path, newline="", encoding="utf-8-sig") as csvfile:
        ...
        for row in reader:
            ...
            # Skip amino acids
            if short_id in CDM_groups["amino_acids"]:
                continue
            id_ = f"{short_id}_{row['stock_concentration_x']}x"
            mw = float(row["mw_g_mol"])
            molecular_weights[id_] = mw
...
def scale_concentrations(to_remove, final_conc, molecular_weights):
    # returns dict of molar concentrations scaled to compensate for the one removed
    final_conc = pd.DataFrame.from_dict(final_conc, orient="index")
    ...
    total_conc = final_conc.sum(axis=0)[1]

    to_remove = dp_utils.assert_list(to_remove)
    removed_vol = sum([final_conc.loc[r].value for r in to_remove])
    scale = total_conc / (total_conc - removed_vol)
    final_conc = final_conc.drop(to_remove, axis=0)
    final_conc["value"] = final_conc["value"].apply(lambda x: x * scale)
    total_conc = final_conc.sum(axis=0)[1]
...
def schedule_CDM_l2o(
    ...
    for components in batch_removals:
        ...
        new_aa_conc = scale_concentrations(
